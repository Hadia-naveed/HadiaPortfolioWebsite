...
setTimeout(() => setIsVisible(true), 10); // small delay to allow transition
  ...
  setTimeout(() => setselectedProject(null), 300); // wait for fade-out animation
};
  ...
  return (
    <section  id='projects' className=' bg-gray-950 mt-10 flex flex-col w-full mx-auto max-w-screen-xl'>
      <div>
        ...
        <h1 className='text-white text-4xl sm:text-5xl font-bold text-center'>Featured{" "}<span className='text-transparent bg-gradient-to-r from-purple-600 to-pink-600 bg-clip-text'>Projects</span></h1>
        <p className='mt-6 font-medium tracking-normal leading-[24px] text-[17px] text-center text-gray-200'>I've worked on several impressive projects that showcase my skills,<br /> and I'm excited to share them with you here.</p>
      </div>
      <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 mt-15 lg:px-30 px-10 mx-auto items-stretch'>
        {
          projects.map((project,key)=>(
            <div key={key} 
            className='group bg-[#1a1717] rounded-lg overflow-hidden shadow-xs flex flex-col h-full transition-transform duration-300 hover:shadow-lg hover:scale-[1.02]'>
              ...
              <div className='p-5 '>
                <div className="flex flex-wrap gap-2 justify-center">
                  {project.tag.map((tags)=>(
                    <span className="px-2 py-1 text-xs font-medium border rounded-md bg-[#1a1717] text-gray-100">
  ...
  <button onClick={()=>{openprojectinfo(project)}} 
